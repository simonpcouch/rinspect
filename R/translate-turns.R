...
translate_turns <- function(chat, sample = 1L) {
  ...
  for (i in seq_along(turns)) {
    ...
    if (turn@role == "assistant") {
      events[[i]] <- translate_assistant_turn(turn)
    } else {
      events[[i]]<- translate_user_turn(turn, sample)
    }
  }
...
translate_user_turn <- function(turn, sample) {
  list(
...
translate_assistant_turn <- function(turn) {
  list(
    # TODO: grab these from ellmer when available?
    timestamp = format(Sys.time(), "%Y-%m-%dT%H:%M:%OS6%z"),
    event = "model",
