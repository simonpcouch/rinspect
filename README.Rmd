...
* **Solvers** are functions that take `input` and return some value approximating `target`, likely wrapping ellmer chats. `generate()` is the simplest scorer in rinspect, and just passes the `input` to the chat's `$chat()` method, returning its result as-is.
...
Any arguments to the solver or scorer can be passed to `$eval()`, allowing for straightforward parameterization of tasks. For example, if I wanted to evaluate `chat_openai()` on this task rather than `chat_anthropic(model = "claude-3-7-sonnet-latest")`, I could write:
...
For an applied example, see the "Getting started with rinspect" vignette at `vignette("rinspect", package = "rinspect")`.
