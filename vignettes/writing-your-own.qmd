---
title: "Writing your own evals for your LLM product"
vignette: >
  %\VignetteIndexEntry{Writing your own evals for your LLM product}
...
In short, inputs should be **natural**. Rather then "setting up" the model with exactly the right context and phrasing, "\[i\]t's important that the dataset... represents the types of interactions that your AI will have in production" [@husain2024judge].
...
-   Consider what metadata you want to keep track of along with your `input` and `target`. Do you want to tag questions with categories? Include a source URL? This isn't stuff that will actually be integrated in the eval, but that will be carried along throughout it to aid you with analysis down the line.
...
-   LLM-as-a-judge: In most situations, you'll want to pass the input, the solver's result, and the grading guidance (`target`) to a model, and ask the model to allot some score. (vitals natively supports factors with levels `I < P < C`, standing for Incorrect, (optional) Partially Correct, and Correct.)
