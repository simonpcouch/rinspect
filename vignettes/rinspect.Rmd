...
The `Task` R6 class allows us to create an evaluation task, composed of the dataset, solver, and scorer. Any dataset is fair game as long as it has columns `input` and `target`. We'll create a new Task with those elements:
...
When we call `$eval()`, the Task will:

1. Run the solver on each input in the dataset. Each `input` from the dataset is sent to Claude. For each input prompt, Claude generates a response that attempts to solve the R coding problem.
2. Score each solution against the target using model-grading.
3. Log the results for later inspection. (When run interactively, `eval()` will display the log in an interactive app by default.)
...
The task also contains score information from the scoring step. We've used `model_graded_qa()` as our scorer, which uses another model to evaluate the quality of our solver's solutions against the reference solutions in the `target` column.
...
The Task object automatically launches the viewer when you call `task$eval()` in an interactive session. You can also view results anytime with `are_task$view()`. You can explore this eval specifically with the following code (TODO: code to grab from github and view).
...
These questions can be explored by creating new Task objects with different solvers and scorers. For example, to compare Claude's performance with OpenAI's GPT-4o:
