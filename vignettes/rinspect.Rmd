...
2) **Solvers** evaluate the `input` in the dataset and produce a final result (hopefully) approximating `target`. In rinspect, the simplest solver is just an ellmer chat (e.g. `ellmer::chat_anthropic(model = "claude-3-7-sonnet-latest")`) wrapped in `generate()`, i.e. `generate(ellmer::chat_anthropic(model = "claude-3-7-sonnet-latest")`), which will call the Chat object's `$chat()` method and return whatever it returns.
...
if (identical(Sys.getenv("IN_PKGDOWN"), "true")) {
  htmltools::tags$iframe(
    src = "../example-logs/rinspect/index.html",
    width = "100%", 
    height = "600px",
...
are_task_data <- inspect_bind(are_task)
...
```{r}
are_task_eval <-
  inspect_bind(are_task, are_task_openai) %>%
  mutate(
...
are_task_eval %>%
  ggplot(aes(x = score, fill = model, group = model)) +
...
are_mod <- polr(score ~ model, data = are_task_eval)
...
```{r}
#| include: false
grade_descriptor <- if (are_mod[["coefficients"]] > 0) "higher" else "lower"
```

The coefficient for `model == "GPT-4o"` is `r round(are_mod[["coefficients"]], 3)`, indicating that GPT-4o tends to be associated with `r grade_descriptor` grades. If a 95% confidence interval for this coefficient contains zero, we can conclude that there is not sufficient evidence to reject the null hypothesis that the difference between GPT-4o and Claude's performance on this eval is zero at the 0.05 significance level.
...
:::callout-note
If we had evaluated this model across multiple epochs, `samples$epoch` could become a "nuisance parameter" in a mixed model.
:::
...
# `pkgdown/assets/`
dest_dir <- here::here("pkgdown/assets/example-logs/rinspect")
inspect_bundle(
