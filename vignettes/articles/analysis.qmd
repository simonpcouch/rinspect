...
* We set `glue::backtick(paste0("epochs = ", length(unique(are_eval$epoch))))` in `Task$new()` so that each sample in `are` is run through the eval multiple times.
...
* **[ANOVA-Type Models](#anova-type-models)**: Frequentist models using ordinal regression
  * **[Independent Data](#independent-data)**: Analyze single-epoch results across one covariate (like LLM or system prompt)
