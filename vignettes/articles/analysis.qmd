...
* We evaluate three models, rather than two.
* We pass `epochs = 3` to `Task$new()` so that each sample in `are` is run through the eval three times.
...
#| label: leaders
#| include: false
# reorder `model` so that the worst-performing is first
leaders <-
  are_eval %>%
  summarize(correct = mean(score == "C"), .by = c(model)) %>%
  arrange(correct)

are_eval <- are_eval %>%
  mutate(model = factor(model, leaders$model))
```
...
* **[ANOVA-Type Models](#anova-type-models)**: Frequentist models using ordinal regression
  * **[Independent Data](#independent-data)**: Analyze single-epoch results across one covariate (like model or system prompt)
  * **[Multiple Hierarchies](#multiple-hierarchies)**: Account for repeated measures across epochs using mixed effects models
* **[Bayesian Hierarchical Models](#bayesian-hierarchical-models)**: More computionally intensive but flexible modeling and natural probability statements
...
* We're comparing results from 3 different **tasks**. The separate tasks, in this case, represent three different models, Anthropic's Claude 4 Sonnet, and OpenAI's GPT 4.1, and Google's Gemini 2.5 Pro. We'll refer to the identifier for the task as `p` (shown in `are_eval` as column `model`).
* Underlying the tasks are a dataset of $n =$ `r nrow(are_eval) / length(unique(are_eval$model))` **samples**. Each sample contains a question to be asked to a model. 
* Once the model answers a given sample, the answer is assigned one of three qualitative, ordinal **scores**: Incorrect (`I`), Partially correct (`P`), or correct (`C`). We'll denote $Y$ as a discrete random variable representing the scores that take on values $1, 2, \ldots c$ (with $c=3$ here).

A completely balanced data set would contain $p \times n$ rows (and does in our case).

With this data structure, the `model` is the systematic effect in the data on which we would like to make inferences about (i.e., is accuracy better for one model, etc.). The samples are the *independent experimental units*; statistically, we believe that the results for sample 1 are independent of sample 2 and so on.

There are two potential levels of hierarchy we need to consider when analyzing evaluation data:

* LLMs are not deterministic; given the same prompt, models will not always return the same answer. To measure variation, we run the same sample multiple times, generating a set of $m$ **epochs**. We thus must account for *repeated measures*, where the epochs have no systematic ordering effect. However, we expect a within-sample correlation to occur. This means that we would expect the $m$ data points within a specific sample to be more likely to correlate with one another than with a data point from a different sample.
...
<!--# TODO: is this new "parallel" interpretation correct? -->
...
We've ordered the models above so that the worst-performing model has an index of $j=1$. This parameterization allows for a direct path to inference since the $exp(\beta_j)$ parameters increase the odds that LLM $j$ performs *better* than the worst model. We can easily estimate null hypotheses that test the two models' differences and/or create confidence intervals for the odds.
...
```{r}
#| label: single-model
single_mod <- clm(score ~ model, data = are_eval_single)
```
...
single_coef <- tidy(single_mod, conf.int = TRUE, conf.level = 0.9) %>%
	mutate(
		model = gsub("model", "", term),
		parameter = ifelse(coef.type == "intercept", "theta", "beta")
	)

single_pred <- 
  predict(single_mod, newdata = leaders %>% select(model))$fit %>% 
  as_tibble() %>% 
  bind_cols(leaders %>% select(model)) %>% 
  relocate(model)
...
single_coef %>% 
  filter(!grepl("\\|", model)) %>% 
  select(model, estimate, lower = conf.low, upper = conf.high) %>% 
  mutate(
...
These are the odds ratios for the different models. Recall that, for the model fit, the LLMs were indexed with `r leaders$model[1]` in the first position. The estimates above are the increase in the odds of being more correct than `r leaders$model[1]`. The fact that all of the intervals include 1.0 indicates that there is not enough evidence to believe that there are differences in any of these LLMs.

### Multiple Hierarchies

How does the analysis change when we incorporate the samples from every epoch? To illustrate the structure we'd like to model, we can visualize the variation in scores for each sample and model similarly to before:
...
To model this structure, we first fit two models. The first is as before, additionally incorporating the random effect `(1|id)`. Note that the random effect is for the question ID rather than the epoch. The second, null model does not account for the `model`.
...
Comparing these two model fits, then, shows us the effect of incorporating the `model` variable on the fit:
...
For this analysis, the odds ratios indicate that the other two models improved accuracy relative to `r leaders$model[1]`, but the overlap of the 90% confidence intervals with 1 suggests that there are no statistically significant differences from the other models.
...
Running this evaluation across multiple epochs also allows us to ask more interesting questions about the evaluation itself (rather than models' performance on it alone), too. **Are some questions harder than others?**
...
bayes_regression_param <- 
  ...
  mutate(
    model = 
      case_when(
        param_name == "b_modelGemini2.5Pro" ~ "Gemini 2.5 Pro",
        param_name == "b_modelClaude4Sonnet" ~ "Claude 4 Sonnet"
      ),
    model = factor(model)
  ) 
...
#| label: posterior
#| warning: false
post_claude <- bayes_regression_param$value[bayes_regression_param$model == "Claude 4 Sonnet"]
o2_le_zero <- mean(abs(post_claude) < 0.05) * 100

bayes_regression_param %>% 
  ggplot(aes(x = value)) + 
  geom_vline(xintercept = 0, lty = 2, col = "red") +
  geom_density(aes(col = model), linewidth = 1, alpha = 1 / 2) +
  labs(x = expression(hat(beta))) +
  theme(legend.position = "top") + 
  scale_color_brewer(palette = "Set2")
```

Since these parameters measure model effectiveness (relative to the baseline model of `r leaders$model[1]`), we can tell what our model thinks that likely values are. It suggests that the GPT 4.1 performs on par with Gemini 2.5 Pro and slightly worse than Claude 4 Sonnet.
...
bayes_reg_summary %>% 
  ...
  fmt_number(columns = c(-model), n_sigfig = 3)
```

The percentile columns are credible intervals which allow us to make statements such as:

> We believe that the true odds ratio comparing `r leaders$model[1]` to Claude 4 Sonnet has a 90% probability of falling between `r signif(bayes_reg_summary$lower_odds[2], 3)` and `r signif(bayes_reg_summary$upper_odds[2], 3)`.
