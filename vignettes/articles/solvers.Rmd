...
The implementation of our custom solver looks almost exactly like `generate()`s implementation, except we add a call to `btw_register_tools()` before calling `ellmer::parallel_chat()`.
...
#| include: false
if (should_eval) {
  scrub_providers_ <- getFromNamespace("scrub_providers", "vitals")
  are_btw <- scrub_providers_(are_btw)
...
if (should_eval) {
  scrub_providers_ <- getFromNamespace("scrub_providers", "vitals")
  are_basic <- scrub_providers_(are_basic)
