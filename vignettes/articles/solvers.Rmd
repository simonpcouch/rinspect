...
```{r}
generate(solver_chat = chat_anthropic(model = "claude-3-7-sonnet-latest"))
```
...
btw_solver <- function(inputs, ..., solver_chat) {
  btw_register_tools(solver_chat, include = "docs")
  # TODO: revert this to use parallel solving (#84)
  res <- list()
  for (input in inputs) {
...
are_task_btw$eval(solver_chat = chat_anthropic(model = "claude-3-7-sonnet-latest"))
```
...
#| include: false
if (should_eval) {
  save(are_task_btw, file = here::here("vignettes/articles/data/solvers-are_task_btw.rda"))
}
```
...
#| include: false
if (should_eval) {
  save(are_task, file = here::here("vignettes/articles/data/solvers-are_task.rda"))
}
```
